...
class DatabaseManager {
  ...
  async loadDatabase() {
    ...
    try {
      ...
      console.error(err);
      this.showError('Gagal load: ' + err.message);
      document.getElementById('jsonViewer').innerHTML = '<code>Error: database.json tidak ditemukan</code>';
    } finally {
  ...
  importJSON(e) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        this.renderJSON(this.data);
        this.updateStats();
        this.showSuccess('JSON diimport');
      } catch {
        this.showError('JSON tidak valid');
      }
    };
  ...
  collapseJSON() {
    if (!this.data) return;
    document.getElementById('jsonViewer').innerHTML = `<code>${Object.keys(this.data).length} keys...</code>`;
  }
  ...
  saveSettingsForm() {
    ...
    this.saveSettings();
    this.applySettings();
    this.showSuccess('Settings tersimpan & realtime');
  }
